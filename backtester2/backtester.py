...
class LongTerm(bt.Strategy):
    ...
    def __init__(self, symbol:str, df:pandas.DataFrame):
        ...
        # KD 隨機震盪指標 5-3-3
        # 找買點
        self.k_buy, self.d_buy = talib.STOCH(
            df['High'],
            df['Low'],
            df['Close'],
            fastk_period=4,
        ...
        # 找賣點
        self.k_sell, self.d_sell = talib.STOCH(
            df['High'],
            df['Low'],
            df['Close'],
            fastk_period=50,
        ...
        # 濾出買賣訊號
        signals_buy = self.k_buy.apply(lambda k: self.__signal(k, 'B'))
        signals_sell = self.k_sell.apply(lambda k: self.__signal(k, 'S'))
        df['K_BUY'] = self.k_buy
        df['D_BUY'] = self.d_buy
        df['K_SELL'] = self.k_sell
        df['D_SELL'] = self.d_sell
        df['Signal_Buy'] = signals_buy
        df['Signal_Sell'] = signals_sell
        df['Signal'] = 0
        for index in range(df.shape[0]):
            if df['Signal_Buy'].iloc[index] > 0:
                df['Signal'].iat[index] = df['Signal_Buy'].iloc[index]
            if df['Signal_Sell'].iloc[index] < 0:
                df['Signal'].iat[index] = df['Signal_Sell'].iloc[index]
    ...
    def __signal(self, k:float, mode:str) -> int:
        if math.isnan(k): return 0
        if k >= 15:
            if k > 95 and mode == 'S':
                return -2
            elif k > 88 and mode == 'S':
                return -1
            else:
                0
        if k < 10 and mode == 'B':
            return 2
        elif k < 15 and mode == 'B':
            return 1
